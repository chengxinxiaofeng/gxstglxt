...
<body > 
   <div style="margin-top:10px;">
            ...
            <div style="margin-top:10px" class="demoTable">
               <button class="layui-btn  layui-btn-sm" data-type="getCheckData"><i class="layui-icon" style="font-size: 30px; ">&#xe640;</i>批量通过  </button>
                <button class="layui-btn layui-btn-sm" onclick="fnRefreshData()" ><i class="layui-icon" style="font-size: 30px; ">&#x1002;</i>刷新  </button>
            </div>
<table class="layui-table" lay-data="{width: '100%', height:'full-100', url:'getVerifyCommonUserInfo.action', page:true, id:'idTest'}" lay-filter="demo">
  <thead>
    <tr>
      <th lay-data="{type:'checkbox', fixed: 'left'}"></th>
      <th lay-data="{field:'STUDENT_ID', width:'120', sort: true, align:'center',fixed: true}">学号</th>
  <!--     <th lay-data="{field:'VIRTUALNAME',  align:'center',width:'90', sort: true}">昵称</th> -->
      <th lay-data="{field:'REALNAME',  width:'100',  align:'center',sort: true}">姓名</th>
      <th lay-data="{field:'GENDER',  width:'70',  align:'center',sort: true ,templet: function(d){
       ...
       return  '<i class=layui-icon my-icon style = font-size: 30px; color: #1E9FFF;>&#xe662;</i>' }
           ...
           <th lay-data="{field:'IMGURL',  width:'70',  align:'center' , event: 'setSign', style:'cursor: pointer;',templet: function(d){
      ...
      <th lay-data="{field:'PHONE',  width:'130',align:'center',sort: true}">手机号码</th>
      <th lay-data="{field:'EMAIL',  width:'170',align:'center', sort: true}">邮箱</th>
      <th lay-data="{field:'REGISTERTIME',  width:'115',align:'center', sort: true}">注册时间</th> 
      <th lay-data="{field:'INSTITUTE_NAME',  width:'205',align:'center', sort: true}">所属学院</th> 
...
<script type="text/html" id="checkboxTpl">
  <!-- 这里的 checked 的状态只是演示 -->
  <input type="checkbox" name="lock" value="{{d.STUDENT_ID}}" title="通过" lay-filter="lockDemo" {{ d.VALIDFLAG == 0 ? 'checked' : '' }}>
</script>
...
layui.use('table', function(){
  ...
  var $$ = layui.$, active = {
    getCheckData: function(obj){ //获取选中数据
     ...
     if(data.length == 0){
    	 layer.msg('请选择要通过的用户', {icon: 5});
    	 return;
     }
     layer.confirm('确认批量通过选中的用户吗？', function(index){  
		     $.ajax({
			    	
			        url: "updateBatchValidFlag.action",        //后台url
			        ...
			        success: function (data) {      //成功，回调函数 	    
			        		fnRefreshData();
			        		if(data == 1){
			        			layer.msg('批量通过用户成功', {icon: 1});
			        			//重新加载数据
			        		}else{
			        			layer.msg('批批量通过用户失败', {icon: 5});
			        		}
			        },
			        error: function (er) {          //失败，回调函数
			        
			        	layer.msg('批批量通过出错', {icon: 5});
			        }
			    });
		     layer.close(index);
     	});
...
function fnSearchUser(){
	...
	if(searchWord==""){
		...
		table.reload('idTest', {
			...
			url:'findVerifyGivenUser.action?searchWord='+searchWord
		});
	}
}
//刷新数据
function fnRefreshData(){
	...
	table.reload('idTest', {
		...
		url:'getVerifyCommonUserInfo.action'
	});
}
		 ...
		 $(this).attr("disabled","true");
		  var student_id = this.value;
			$.ajax({
			        ...
			        url: "updateValidFlag.action?student_id="+student_id,        //后台url
			        type: "get",                   //类型，POST或者GET
			        dataType: 'html',              //数据返回类型，可以是xml、json等
			        success: function (data) {      //成功，回调函数 	    
			        	if(data==1){
			        		layer.msg('审核通过学号为['+student_id+']的用户成功~', {icon: 1});
			        		fnRefreshData();
			        	}else{
			        		layer.msg('审核通过学号为['+student_id+']的用户失败', {icon: 5});
			        	}
			        		
			        		
			        },
			        error: function (er) {          //失败，回调函数
			        	layer.msg('审核出错', {icon: 5});
			        }
			    });
	  });
	});
