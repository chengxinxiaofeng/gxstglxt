...
<body >
	    <div class="div-notice" style="margin-top:10px">
          <div class="notic-head" style="font-size:20px">
             	 当前位置:<a href="javascript:;" onclick="parent.fnOpenPage(1)" style="cursor:pointer">首页</a>&nbsp;/&nbsp;<a class="notice-onclick" href="javascript:;">新闻</a>
              	<hr>
          </div>
      </div>
      <div>
...
function fnGetNoticeData(page){	
	$.ajax({   	
        url: "getAllNewsData.action?page="+page,        //后台url
...
function fnGetGivenNoticeInfo(news_id){
	$.ajax({   	
        ...
        success: function (data) {      //成功，回调函数 	     
        		...
        		$(".notic-head").empty();
        		var temp = "当前位置:<a href='javascript:;' onclick='parent.fnOpenPage(1)' style='cursor:pointer'>首页</a>&nbsp;/&nbsp;<a  href='javascript:;' onclick='fnChangeNoticeUrl()' >新闻</a>&nbsp;/&nbsp;新闻正文<hr>"
...
function fnChangeNoticeUrl(){
	...
	$("#demo1").css("display","");
	$(".notic-head").empty();
	var temp = "当前位置:<a href='javascript:;' onclick='parent.fnOpenPage(1)' style='cursor:pointer'>首页</a>&nbsp;/&nbsp;<a  href='javascript:;'  >新闻</a><hr>"
	$(".notic-head").append(temp);
	 fnSetTest();
}
